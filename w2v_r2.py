...
def train_batch_sg(model, sentences, alpha, work=None):
    ...
    for sentence in sentences:
        ...
        for pos, word in enumerate(word_vocabs):
            ...
            for pos2, word2 in enumerate(word_vocabs[pos:], pos):
...
def train_batch_cbow(model, sentences, alpha, work=None, neu1=None):
    ...
    for sentence in sentences:
        word_vocabs = [model.vocab[w] for w in sentence if w in model.vocab and
                       model.vocab[w].sample_int > model.random.rand() * 2**32]
        for pos, word in enumerate(word_vocabs):
            window_pos = enumerate(word_vocabs[pos:], pos)
...
#class Word2Vec(object):  # without save/load
    ...
    def show_similar_items(self, idx):
        print('----- http://tabelog.com' + self.index2word[idx])
        for name, score in self.most_similar(positive=self.index2word[idx]):
            print('{0:.3f}'.format(score), 'http://tabelog.com' + name)
